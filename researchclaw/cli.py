...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if no_graceful_degradation:
        ...
        new_research = _dc.replace(config.research, graceful_degradation=False)
        config = _dc.replace(config, research=new_research)
    ...
    if from_stage_name:
        try:
            ...
            print(f"Error: unknown stage '{from_stage_name}'. Valid stages: {valid}", file=sys.stderr)
            return 1
    elif resume:
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    _candidates = [
        Path(__file__).resolve().parent.parent / EXAMPLE_CONFIG,
        Path.cwd() / EXAMPLE_CONFIG,
        Path(__file__).resolve().parent / "data" / EXAMPLE_CONFIG,
    ]
    ...
    content = content.replace('provider: "openai-compatible"', f'provider: "{provider}"')

    if provider == "acp":
        content = content.replace('base_url: "https://api.openai.com/v1"', 'base_url: ""')
        ...
        if api_key_env:
            content = content.replace('api_key_env: "OPENAI_API_KEY"', f'api_key_env: "{api_key_env}"')
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_p.add_argument("--topic", "-t", help="Override research topic")
    run_p.add_argument("--config", "-c", default=None, help="Config file")
    run_p.add_argument("--output", "-o", help="Output directory")
    run_p.add_argument("--from-stage", help="Start from a specific stage")
    run_p.add_argument("--auto-approve", action="store_true", help="Auto-approve gate stages")
    run_p.add_argument("--skip-preflight", action="store_true", help="Skip LLM preflight check")
    run_p.add_argument("--resume", action="store_true", help="Resume from last checkpoint")
    run_p.add_argument("--skip-noncritical-stage", action="store_true",
                       help="Skip noncritical stages on failure")
    run_p.add_argument("--no-graceful-degradation", action="store_true",
                       help="Disable graceful degradation")

    val_p = sub.add_parser("validate", help="Validate config file")
    val_p.add_argument("--config", "-c", default=None, help="Config file")
    val_p.add_argument("--no-check-paths", action="store_true", help="Skip path existence checks")

    doc_p = sub.add_parser("doctor", help="Check environment health")
    doc_p.add_argument("--config", "-c", default=None, help="Config file")
    doc_p.add_argument("--output", "-o", help="Write JSON report to file")

    init_p = sub.add_parser("init", help="Create config from example template")
    init_p.add_argument("--force", action="store_true", help="Overwrite existing config")

    sub.add_parser("setup", help="Check and install optional tools")

    rpt_p = sub.add_parser("report", help="Generate run report")
    rpt_p.add_argument("--run-dir", required=True, help="Path to run artifacts")
    rpt_p.add_argument("--output", "-o", help="Write report to file")
