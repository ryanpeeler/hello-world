...
_NEW_PARAM_MODELS = frozenset({
    "o3", "o3-mini", "o4-mini", "gpt-5", "gpt-5.1", "gpt-5.2", "gpt-5.4",
})
...
class LLMConfig:
    ...
    fallback_models: list[str] = field(default_factory=lambda: ["gpt-4.1", "gpt-4o-mini"])
    max_tokens: int = 4096
...
class LLMClient:
    ...
    def preflight(self) -> tuple[bool, str]:
        ...
        try:
            _ = self.chat(
                ...
                max_tokens=min_tokens, temperature=0,
            )
            ...
            try:
                ...
                if status == 400:
                    _transient_400 = any(
                        ...
                        for kw in ("rate limit", "ratelimit", "overloaded",
                                   "temporarily", "capacity", "throttl", "too many", "retry")
                    )
                ...
                if status in (400, 429, 500, 502, 503, 504, 529):
                    ...
                    logger.info(
                        ...
                        attempt + 1, self.config.max_retries, model, status, delay,
                    )
                    time.sleep(delay)
                    continue
                raise
    ...
    def _raw_call(
        ...
        if self._anthropic:
            ...
            body: dict[str, Any] = {
                "model": model, "messages": msgs, "temperature": _temp,
            }
            ...
            try:
                ...
                if self.config.fallback_url:
                    logger.warning(
                        ...
                        self.config.fallback_url, exc,
                    )
                    fallback_url = f"{self.config.fallback_url.rstrip('/')}/chat/completions"
...
def create_client_from_yaml(yaml_path: str | None = None) -> LLMClient:
    ...
    return LLMClient(
        LLMConfig(
            ...
            fallback_models=llm_section.get("fallback_models", ["gpt-4.1", "gpt-4o-mini"]),
        )
    )
