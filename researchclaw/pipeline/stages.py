...
PHASE_MAP: dict[str, tuple[Stage, ...]] = {
    ...
    "B: Literature Discovery": (
        Stage.SEARCH_STRATEGY, Stage.LITERATURE_COLLECT,
        Stage.LITERATURE_SCREEN, Stage.KNOWLEDGE_EXTRACT,
    ),
    "C: Knowledge Synthesis": (Stage.SYNTHESIS, Stage.HYPOTHESIS_GEN),
    "D: Experiment Design": (
        Stage.EXPERIMENT_DESIGN, Stage.CODE_GENERATION, Stage.RESOURCE_PLANNING,
    ),
    ...
    "G: Paper Writing": (
        Stage.PAPER_OUTLINE, Stage.PAPER_DRAFT,
        Stage.PEER_REVIEW, Stage.PAPER_REVISION,
    ),
    "H: Finalization": (
        Stage.QUALITY_GATE, Stage.KNOWLEDGE_ARCHIVE,
        Stage.EXPORT_PUBLISH, Stage.CITATION_VERIFY,
    ),
}
...
def gate_required(
    stage: Stage, hitl_required_stages: Iterable[int] | None = None,
) -> bool:
...
def advance(
    ...
    *,
    ...
    if event is TransitionEvent.START and status in {
        ...
        return TransitionOutcome(stage=stage, status=StageStatus.RUNNING, next_stage=stage)
