...
class QualityReport:
    ...
    def to_dict(self) -> dict[str, object]:
        match_rows: list[dict[str, object]] = [
            {"pattern": m.pattern_desc, "line": m.line_number, "excerpt": m.excerpt}
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        for pattern, desc in _TEMPLATE_PATTERNS:
            for m in re.finditer(pattern, stripped):
                ...
                matches.append(
                    TemplateMatch(pattern_desc=desc, line_number=line_num, excerpt=excerpt)
                )
    return matches


def compute_template_ratio(text: str) -> float:
    """Estimate fraction of text that is template/placeholder content."""
    if not text.strip():
        return 0.0
...
def assess_quality(text: str) -> QualityReport:
    ...
    logger.debug(
        ...
        report.total_lines, report.total_chars, report.match_count, report.template_ratio,
    )
    return report
